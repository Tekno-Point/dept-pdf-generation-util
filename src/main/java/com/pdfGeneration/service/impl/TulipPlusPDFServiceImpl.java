...
import com.pdfGeneration.service.TulipPlusPDFService;
...
@Service
public class TulipPlusPDFServiceImpl extends NomineeAddendumPDF implements TulipPlusPDFService {
    ...
    public TulipPlusPDFServiceImpl(PDFUtility pdfUtility, JsonUtility jsonUtility) {
        this.pdfUtility = pdfUtility;
        this.jsonUtility = jsonUtility;
    }
    
    @Override
    public byte[] generateTulipPlusPdf(String tulipPlusPdfReqObj) {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        try {
            JsonObject userData = jsonUtility.getJsonObject(tulipPlusPdfReqObj);
            ...
            pdfDocumentInfo.setTitle("Tulip Plus Application Form");
            ...
            if (permanentaddressline1.length() > 0) {
                Table address1Block = pdfUtility.codeTable(permanentaddressline1, pointColumnWidths);
            ...
            if (permanentaddressline2.length() > 0) {
                Table address2Block = pdfUtility.codeTable(permanentaddressline2, pointColumnWidths);
            ...
            if (permanentaddressline3.length() > 0) {
                Table address3Block = pdfUtility.codeTable(permanentaddressline3, pointColumnWidths);
            ...
            if (permanentlandmark.length() > 0) {
                Table address3Block = pdfUtility.codeTable(permanentlandmark, pointColumnWidths);
                addressBlock.addCell(new Cell().add(address3Block).setBorder(Border.NO_BORDER));
            }

            pointColumnWidths = new float[permanentcity.length()];
            for (int i = 0; i < permanentcity.length(); i++) {
                pointColumnWidths[i] = 20F;
            }
            if (permanentcity.length() > 0) {
                Table address3Block = pdfUtility.codeTable(permanentcity, pointColumnWidths);
                addressBlock.addCell(new Cell().add(address3Block).setBorder(Border.NO_BORDER));
            } else {
                addressBlock.addCell(new Cell().add("").setBorder(Border.NO_BORDER));
            }
            ...
            if (state.length() > 0) {
                Table address3Block = pdfUtility.codeTable(state, pointColumnWidths);
                addressBlock.addCell(new Cell().add(address3Block).setBorder(Border.NO_BORDER));
            } else {
                addressBlock.addCell(new Cell().add("").setBorder(Border.NO_BORDER));
            }
            ...
            p.add(new Text("\n*Receive alerts through SMS andWhatsApp for this proposal / policy").setFontSize(5F));
...
//            p = new Paragraph();
            ...
            p.add(new Text("(Proposer)").setFontSize(5F));
            ...
            p=new Paragraph("Income: ");
            p.add(new Text("\n(Annual) ").setFontSize(5F));
            otherDetails.addCell(new Cell().add(p).setBorder(Border.NO_BORDER).setTextAlignment(TextAlignment.LEFT));
            ...
            p.add(new Text("(Please provide Form 60, if PAN is not available)").setFontSize(5F));
...
//            }
            ...
            table.addCell(new Cell().add(p).setPaddingLeft(10F).setVerticalAlignment(VerticalAlignment.MIDDLE).setTextAlignment(TextAlignment.LEFT).setBorder(Border.NO_BORDER));
            ...
            boolean adbRider = jsonUtility.getBooleanKeyValue("ADBRider", planDetailObj);
            planTerm=adbRider ? jsonUtility.getJsonKeyValue("adbPlanTerm",planDetailObj): "";
            premiumPlayingTerm = adbRider ? jsonUtility.getJsonKeyValue("premiumPayingTerm",planDetailObj) : "";
            premiumInstallment= adbRider ? jsonUtility.getJsonKeyValue("adbPremiumWithTax",planDetailObj): "";
            sumAssured= adbRider ? jsonUtility.getJsonKeyValue("adbSumAssured",planDetailObj) : "";
            //2 new added plan
            ...
            boolean tpdRider = jsonUtility.getBooleanKeyValue("TPDRider", planDetailObj);
            planTerm= tpdRider ? jsonUtility.getJsonKeyValue("tpdPlanTerm",planDetailObj): "";
            premiumPlayingTerm = tpdRider ? jsonUtility.getJsonKeyValue("premiumPayingTerm",planDetailObj) : "";
            premiumInstallment = tpdRider ? jsonUtility.getJsonKeyValue("tpdPremiumWithTax",planDetailObj): "";
            sumAssured = tpdRider ? jsonUtility.getJsonKeyValue("tpdSumAssured",planDetailObj) : "";
...
//            fundsDetails.addCell(new Cell().add(jsonUtility.getJsonKeyValue("allocatefund1",planDetailObj)).setTextAlignment(TextAlignment.CENTER));
//            fundsDetails.addCell(new Cell().add("Value ((SFIN:ULIF013010910VALUEFUND0143))"));
//            fundsDetails.addCell(new Cell().add("0").setTextAlignment(TextAlignment.CENTER));
...
//            fundsDetails.addCell(new Cell().add(jsonUtility.getJsonKeyValue("debt1",planDetailObj)).setTextAlignment(TextAlignment.CENTER));
...
//            fundsDetails.addCell(new Cell().add(jsonUtility.getJsonKeyValue("flexiCapEquityFund",planDetailObj)).setTextAlignment(TextAlignment.CENTER));
...
//            fundsDetails.addCell(new Cell().add(jsonUtility.getJsonKeyValue("SustainableEquityFund",planDetailObj)).setTextAlignment(TextAlignment.CENTER));
//            fundsDetails.addCell(new Cell().add("   "));
//            fundsDetails.addCell(new Cell().add("   "));
//            fundsDetails.addCell(new Cell().add("   "));
//            fundsDetails.addCell(new Cell().add("   "));
//            fundsDetails.addCell(new Cell(1,6).add("Liquid1 fund is not available at inception"));
//            fundsDetails.addCell(new Cell(1,6).add("For Fund Transfer Strategy, please select one Equity Oriented Fund and one Debt Oriented Fund."));
//            //fundsDetails.addCell(new Cell(1,6).add("Liquid1 fund is not available at inception"));
...
//            table.addCell(new Cell().add(p).setBorder(Border.NO_BORDER));
            ...
            Table secondaryLifeAssuredDetails = this.getSecondaryLifeAssuredDetails(applicationNumber,imgUnchecked,innerProposer);
...
/*            medicalLifestyleQ = new Table(new float[]{750F, 150F});
            ...
            p.add("     Weight in kg:   ");
            p.add(new Text("  " + jsonUtility.getJsonKeyValue("weight", secondaryHealthObj) + "  ").setUnderline());
            ...
            Table proposerSignatureDetails = new Table(new float[]{80F, 50F, 80F, 50F, 80F, 70F});
...
//            signatureDeclarants = new Table(new float[]{350F, 300F, 300F, 150F});
...
//            signatureDeclarants.addCell(new Cell().add("Signature or thumb impression of the secondary life to be assured").setBorder(Border.NO_BORDER));
//            signatureDeclarants.addCell(new Cell().add(new Paragraph("").setUnderline()).setBorder(Border.NO_BORDER));
//            table.addCell(new Cell().add(signatureDeclarants).setBorder(Border.NO_BORDER));
...
//            JsonObject haveDisabilityObj = jsonUtility.getJsonObjectByKey("have_disability", medicalDisabilityObj);
            ...
            logger.info("Exception occurs in Tulip Plus pdf generation method:", e);
            return null;
        }
    }
    ...
    public Table getLifeAssuredDetailDifferFromProposer(Image imgChecked,Image imgUnchecked,JsonObject insuredPersonBasicDetailObj,JsonObject secondaryPersonalDetailObj,JsonObject secondaryEmploymentDetailObj,JsonObject secondaryFatcaDetailObj,Set<String> otherSetOccupation,boolean myself,Table innerProposer){
        ...
        if (!myself) {
            ...
            proposerMergedcell = new Cell(1, 2);
            contactTable = new Table(new float[]{200F, 600F, 200F, 420F});
            ...
            Table tableCodes;
            float[]pointColumnWidths = new float[lifeaAssuredDoB.length()];
...
//                otherDetails.addCell(new Cell().add(imgUnchecked).setBorder(Border.NO_BORDER));
    ...
    public Table getSecondaryLifeAssuredDetails(String applicationNo,Image imgUnchecked,Table innerProposer){
